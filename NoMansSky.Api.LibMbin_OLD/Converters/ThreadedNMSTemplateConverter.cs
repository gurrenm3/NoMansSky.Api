    ...
    public class ThreadedNMSTemplateConverter : IMemoryConverter
    {
        ...
        public object GetValue(long address, Type valueType)
        {
            ...
            Parallel.ForEach(fields, field =>
            {
                ...
                if (value == null)
                {
                    ConsoleUtils.LogWarning($"{nameof(ThreadedNMSTemplateConverter)}: The field \"{field.Name}\" is is null. Skipping...");
                }
                else
                {
        ...
        public void SetValue(long address, object valueToSet)
        {
            if (valueToSet == null)
            {
                ConsoleUtils.LogError($"{nameof(ThreadedNMSTemplateConverter)}: Failed to set value because valueToSet is null");
                return;
            }
        ...
        public bool CanConvert(Type typeToCheck)
        {
            if (typeToCheck == null)
            {
                ConsoleUtils.LogError($"{nameof(ThreadedNMSTemplateConverter)} can't check if this type can be converted," +
