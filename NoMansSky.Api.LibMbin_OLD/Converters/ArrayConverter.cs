...
namespace NoMansSky.Api
{
    ...
    public class ArrayConverter : IMemoryConverter
    {
        ...
        public bool CanConvert(Type typeToCheck)
        {
            if (typeToCheck == null)
            {
                ConsoleUtils.LogError($"{nameof(ArrayConverter)} can't check if this type can be converted," +
        ...
        public Array GetValue(long address, Type arrayType, int arrayLength)
        {
            ...
            for (int i = 0; i < arrayLength; i++)
            {
                ...
                if (arrayItem == null)
                {
                    ConsoleUtils.LogWarning($"{nameof(ArrayConverter)}: Field value is null. Skipping...");
                    continue;
                }
        ...
        {
            ConsoleUtils.LogError("Cannot use GetValue without providing an array length. Please use overloads that accept array length.");
            return null!;
        }
