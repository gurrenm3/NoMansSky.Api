...
namespace NoMansSky.Api
{
    ...
    public unsafe class ListConverter : IMemoryConverter
    {
        ...
        public bool CanConvert(Type typeToCheck)
        {
            if (typeToCheck == null)
            {
                ConsoleUtils.LogError($"{nameof(ListConverter)} can't check if this type can be converted," +
        ...
        public object GetValue(long address, Type valueType)
        {
            if (address <= 0)
            {
                ConsoleUtils.LogError($"{nameof(ListConverter)}: Can't get List because address was {address} and is not valid");
                return null!;
            }
            ...
            if (elementType == null)
            {
                ConsoleUtils.LogError($"{nameof(ListConverter)}: Failed to get ElementType for List");
                return null!;
            }
        ...
        public void SetValue(long address, object valueToSet)
        {
            ...
            {
                ConsoleUtils.LogError("The API currently does not support changing the size of lists in memory." +
